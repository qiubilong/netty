...
public abstract class AbstractBootstrap<B extends AbstractBootstrap<B, C>, C extends Channel> implements Cloneable {
    ...
    private volatile ChannelHandler handler;  /* 客户端 - pipeLine自定义初始化 */
    ...
    private ChannelFuture doBind(final SocketAddress localAddress) {
        ...
        if (regFuture.isDone()) {
            ...
            regFuture.addListener(new ChannelFutureListener() {//绑定线程&Selector成功后
                @Override
    ...
    final ChannelFuture initAndRegister() {
        ...
        ChannelFuture regFuture = config().group().register(channel); /* 3、 Channel绑定EventLoop线程 & 注册NIO多路复用 & 执行ChannelInitializer -- MultithreadEventLoopGroup --> SingleThreadEventLoop */
        if (regFuture.cause() != null) {
    ...
    private static void doBind0(
        ...
        channel.eventLoop().execute(new Runnable() {//异步执行
            @Override
            public void run() {
